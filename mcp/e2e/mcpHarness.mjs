...
async function waitForApi(apiBase, timeoutMs = 20000) {
  const deadline = Date.now() + timeoutMs
  while (Date.now() < deadline) {
    try {
      // Any routed response proves the listener is up; 404 is fine.
      await fetch(`${apiBase}/api/registry/services`)
      return
    } catch {
      await sleep(150)
    }
  }
  throw new Error(`archd did not start on ${apiBase} within ${timeoutMs}ms`)
}
